...
group.add_argument(
    ...
    metavar="NAME",
    default="test",
    help="dataset validation split (default: test)",
)
group.add_argument(
    "--dataset-download",
    action="store_true",
...
def main():
    ...
    #     print(type(dataset_train.filename(index=args.dataset)))
    #     print('index:',dataset_train.filename(index=args.dataset))
    ...
    if args.jsd_loss:
        ...
        # train_loss_fn = nn.MultiLabelSoftMarginLoss().cuda()
        # train_loss_fn = nn.MultiMarginLoss(margin=0.2).cuda
        # train_loss_fn = nn.TripletMarginLoss(margin=0.2).cuda
        # all_class_names_4 = {0: '崇宁通宝_仰宝', 1: '崇宁通宝_俯宝', 2: '崇宁通宝_昂宝', 3: '崇宁通宝_正宝'}
        # 崇宁通宝_正宝 20
        # 崇宁通宝_俯宝 2
        # 崇宁通宝_昂宝 7
        # 崇宁通宝_仰宝 8
        # alpha = torch.tensor([0.2, 0.5, 0.2, 0.1])
        # train_loss_fn = nn.FocalLoss(alpha=None, gamma=5, num_classes=args.num_classes).cuda
        train_loss_fn = losses.ArcFaceLoss().cuda
    train_loss_fn = train_loss_fn.to(device=device)
    # validate_loss_fn = nn.MultiLabelSoftMarginLoss().to(device=device)
    # validate_loss_fn = nn.MultiMarginLoss(margin=0.1).to(device=device)
    validate_loss_fn = nn.CrossEntropyLoss().to(device=device)
    # validate_loss_fn = nn.TripletMarginLoss(margin=0.2).to(device=device)
